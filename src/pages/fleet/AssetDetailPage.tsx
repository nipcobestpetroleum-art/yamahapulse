  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(300px,.8fr)]">
        ...
        <div className="space-y-4">
          <Card className="border-border bg-card/60"><CardHeader><CardTitle className="text-sm font-semibold">Live telemetry</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3"><Field label="Speed" value={latest?.speed != null ? `${Math.round(latest.speed)} km/h` : "Unknown"} icon={<Gauge className="h-3.5 w-3.5" />} /><Field label="Ignition" value={latest?.ignition == null ? "Unknown" : latest.ignition ? "On" : "Off"} icon={<Zap className="h-3.5 w-3.5" />} /><Field label="Battery" value={latest?.battery_level != null ? `${Math.round(latest.battery_level)}%` : "Unknown"} icon={<Battery className="h-3.5 w-3.5" />} /><Field label="Last seen" value={latest ? format(new Date(latest.recorded_at), "HH:mm:ss") : "Unknown"} icon={<Radio className="h-3.5 w-3.5" />} /></CardContent></Card>
