...
export function LiveMapCanvas({ vehicles, selectedKey, onSelectVehicle }: Props) {
  const markers = useMemo<MapMarkerSpec[]>(() => vehicles.filter((vehicle) => Number.isFinite(vehicle.position.latitude) && Number.isFinite(vehicle.position.longitude) && !(vehicle.position.latitude === 0 && vehicle.position.longitude === 0)).map((vehicle) => {
    ...
    const roadPoint = status === "MOVING" && vehicle.trail.length > 0 ? vehicle.trail[vehicle.trail.length - 1] : null;
    const bearing = vehicle.position.course ?? trailBearing(vehicle.trail);
    return { id: vehicle.key, lat: roadPoint?.[0] ?? vehicle.position.latitude, lng: roadPoint?.[1] ?? vehicle.position.longitude, bearing, title: vehicle.vehicleName, icon: "bike", color: colors[status], scale: selectedKey === vehicle.key ? 10 : 8, snippet: [vehicle.registration ?? "Unregistered vehicle", TELEMETRY_STATUS_LABELS[status], vehicle.position.speed != null ? `${Math.round(vehicle.position.speed)} km/h` : "Speed unavailable", vehicle.position.place_name ?? vehicle.position.address ?? "Location unavailable", `${vehicle.position.location_source ?? "GNSS"}${vehicle.position.location_stale ? " · stale" : ""}`, `GPS ${format(new Date(vehicle.position.recorded_at), "dd MMM yyyy, HH:mm:ss")}`] };
