...
export function MapboxMapCanvas({ token, markers, polylines, selectedMarkerId, onMarkerClick, onMapClick, viewRequest, fitNonce, onViewChange }: MapboxMapCanvasProps) {
  ...
  return <div className="relative overflow-hidden rounded-xl border border-border bg-card/40">
    ...
    {status === "ready" && <div className="pointer-events-none absolute left-3 top-3 flex gap-2"><div className="pointer-events-auto rounded-lg border border-border bg-background/80 px-3 py-2 text-xs font-medium backdrop-blur">Mapbox · {markers.length} marker{markers.length === 1 ? "" : "s"}</div><Button variant="outline" size="sm" className="pointer-events-auto bg-background/80 backdrop-blur" onClick={locate}><Crosshair className="mr-2 h-4 w-4" />GPS</Button><Button variant="outline" size="sm" className="pointer-events-auto bg-background/80 backdrop-blur" onClick={() => mapRef.current?.getContainer().scrollIntoView({ behavior: "smooth", block: "center" })}><Maximize2 className="h-4 w-4" /></Button></div>}
  </div>;
}
